...
참조는 동일하지만 내부 데이터가 변경된 경우라면 [배열이나 객체 내부 변화 감지하기](#배열과-객체의-변화-감지하기) 섹션을 참고하세요.
...
중요: 서드 파티 파이프 중에 성능 저하가 우려되는 파이프는 TypeScript 클래스와 `name`에 `Impure` 접미사를 붙여서 경고하는 경우도 있습니다.
