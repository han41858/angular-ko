...
## 트리거로 지연 로딩 제어하기 {#controlling-deferred-content-loading-with-triggers}
...
| 트리거                        | 설명                                                              |
| ----------------------------- | ----------------------------------------------------------------- |
| [`idle`](#idle)               | 브라우저가 대기 상태일 때 동작합니다. 타입아웃 옵션을 지원합니다. |
...
[`requestIdleCall`](https://developer.mozilla.org/docs/Web/API/Window/requestIdleCallback)에 밀리초 단위의 타이머를 지정할 수 있습니다.
별도 콜백이 없다면 지정된 시간에 타이머가 동작합니다.
...
`IdleService`를 구현하고 이 객체를 `provideIdleServiceWith` 함수로 프로바이더에 등록하면 `idle` 트리거를 커스터마이징 할 수 있습니다.

```ts
@Service()
class CustomIdleService implements IdleService {
  requestOnIdle(callback: (deadline?: IdleDeadline) => void, options?: IdleRequestOptions) {
    // idle 스케쥴 로직을 여기에 작성합니다.
  }

  cancelOnIdle(id: number) {
    // 커스텀 idle 취소 로직을 여기에 작성합니다.
  }
}
...
서버에서 `@defer` 블록을 렌더링하려면 [증분 하이드레이션](/guide/incremental-hydration)이나 `hydrate` 트리거를 사용해야 합니다.
...
`@defer`를 사용하는데 별도로 지연 로딩 청크가 생성되지 않는다면, 지연 로딩 컴포넌트를 어떻게 불러오는지 확인해 보세요.
배럴 파일(`index.ts`)로 로딩하는 것이 흔한 원인입니다.
이런 경우는 번들러가 배럴 파일을 단일 보율로 인식하고 모든 항목을 함께 유지하기 때문에, `@defer` 사용 여부와 관계없이 컴포넌트가 메인 번들 파일에 포함됩니다.
...
```typescript
// parent.component.ts
import {HeavyComponent} from './index'; // 다른 컴포넌트도 함께 로드합니다.
...
해결방법은 간단합니다.
컴포넌트가 정의된 파일 자체를 직접 로드하세요:

```typescript
import {HeavyComponent} from './heavy.component';
```

이렇게 작성하면 별도 청크 파일을 정상적으로 생성하며, 지연 로딩도 제대로 동작합니다.
