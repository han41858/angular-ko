...
템플릿 조각을 렌더링하면, 이 템플릿 조각이 렌더링된 위치가 아니라, **템플릿을 선언한 위치** 에 있는 인젝터 컨텍스트가 사용됩니다.
커스텀 인젝터를 등록하면 이 인젝터를 오버라이딩 할 수 있습니다.

<!--
### Using `NgTemplateOutlet` {#using-ngtemplateoutlet-with-injectors}
-->

### `NgTemplateOutlet` 활용하기 {#using-ngtemplateoutlet-with-injectors}
...
`ngTemplateOutletInjector` 입력 프로퍼티로 커스텀 `Injector`를 전달할 수 있습니다:
...
`ngTemplateOutletInjector`에 문자열로 `'outlet'`을 지정하면 템플릿이 선언된 위치의 인젝터 대신 내장 뷰에 인젝터를 상속할 수 있습니다.
...
개별 `node` 템플릿의 재귀적 렌더링은 이 템플릿을 감싸는 `<item-component>`의 인젝터를 상속받으므로, 개별 중첩 계층에서는 부모 컴포넌트의 프로바이더에 접근할 수 있습니다.

참고: 이 방식은 재귀적인 구조를 구성하거나 컴포넌트 트리에서 프로바이더에 접근해야 하는 상황에 유용합니다.

<!--
### Using `ViewContainerRef` {#using-viewcontainerref-with-injectors}
-->

### `ViewContainerRef` 활용하기 {#using-viewcontainerref-with-injectors}
