...
화면에 표시되는 가로-세로 비율은 이미지 자체의 가로-세로 비율과 같아야 합니다.
...
아래 예제 코드는 URL 인자로 전달되는 `src`, `width`, `height`, 에 `https://example.com`를 붙이는 커스텀 로더 함수입니다.
...
그리고 `ImageLoaderConfig` 객체에는 `src` 프로퍼티가 존재하며, `width`, `height`, `loaderParams` 프로퍼티는 옵션 인자입니다.
...
내장 로더는 Cloudinary, Cloudflare, ImageKit, Imgix를 지원하며, `loaderParams`로 `transform` 프로퍼티를 지정할 수 있습니다.
이 프로퍼티를 사용하면 CDN을 통해 이미지를 변환할 수 있습니다.

`transform` 프로퍼티에는 두 가지 형식을 사용할 수 있습니다:
...
참고: `transform` 프로퍼티는 Netlify 로더를 지원하지 않기 때문에, Netlify CDN에는 변환 인자를 전달할 수 없습니다.
