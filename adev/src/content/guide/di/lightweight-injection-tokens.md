...
그런데 `<lib-header>`는 선택 사항이기 때문에, 가장 단순하게 구성하면 `<lib-card></lib-card>` 라는 템플릿으로 사용할 수 있습니다.
이 경우, `<lib-header>`는 사용되지 않으며, 트리 셰이킹이 될 것으로 예상하겠지만, 실제로는 그렇지 않습니다.
왜냐하면 `LibCardComponent`는 다음과 같이 `LibHeaderComponent`를 참조하기 때문입니다:

```ts
@ContentChild(LibHeaderComponent) header: LibHeaderComponent;
```

- `header: LibHeaderComponent;`에서는 `LibHeaderComponent`가 타입 역할로 사용되었습니다.
- `@ContentChild(LibHeaderComponent)`에서는 `LibHeaderComponent`가 값 역할로 사용되었습니다.
...
예제 코드에서 `LibHeaderComponent` 토큰은 값 역할로 사용되었기 때문에 컴파일 이후에도 코드에 남습니다.
그러면 애플리케이션 어디에서도 `<lib-header>`를 사용하지 않더라도 이 컴포넌트는 트리 셰이킹되지 않습니다.
`LibHeaderComponent`의 코드, 템플릿, 스타일 파일의 크기가 크다면, 클라이언트 애플리케이션의 빌드 결과물 크기에도 영향을 주게 됩니다.
...
- 토큰이 생성자 의존성 주입에서 타입을 지정하기 위해 사용된 경우

아래 예제처럼 `OtherComponent`를 사용하면 트리 셰이킹되지 않고 빌드 결과물에 남습니다:
...
`constructor(@Optional() other: OtherComponent)`라는 코드는 실제로 `constructor(@Optional() @Inject(OtherComponent) other)`라고 작성한 것과 같습니다.
그래서 이 토큰은 값 역할로 사용되었기 때문에 트리 셰이킹이 동작해도 코드에 남습니다.

참고: 라이브러리가 서비스를 제공할 때는 반드시 [트리 셰이킹이 가능한 프로바이더](guide/di/dependency-injection#providing-dependency)를 사용해야 하며, 의존성 객체도 컴포넌트나 모듈 계층이 아니라 최상위 계층에 등록해야 합니다.
...
이 예제에서 `LibCardComponent`의 구현체는 더이상 `LibHeaderComponent`를 타입으로도, 값으로도 사용하지 않습니다.
따라서 `LibHeaderComponent`는 완전하게 트리 셰이킹이 가능합니다.
...
대신, `LibHeaderToken` 추상 클래스의 구현체는 `LibHeaderComponent` 가 됩니다.
...
이제 `LibCardComponent`에서 `LibHeaderComponent` 대신 `LibHeaderToken`를 쿼리해 봅시다.
아래 예제는 `LibCardComponent`가 실제 `LibHeaderComponent`를 직접 호출하지 않아도 `LibHeaderComponent`를 활용할 수 있습니다:
...
Angular 스타일 가이드에 따르면, 컴포넌트는 "Component"라는 접미사를 붙이도록 권장합니다.
"LibHeaderComponent"와 같은 식입니다.

컴포넌트는 토큰과 관련되어 있지만 둘을 명확하게 구분해야 ㅎ바니다.
따라서, 컴포넌트에 "Component" 접미사를 붙이는 것과 비슷하게 "`Token`" 접미사를 붙이는 것을 권장합니다.
