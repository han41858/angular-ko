...
- `readonly header: Signal<LibHeader|undefined>` 에서는 `LibHeader` 타입 역할로 사용되었습니다.
...
`inject(CustomOther)`라는 코드에서 `CustomOther`는 변수 인자로 사용되었습니다.
...
아래 예제에서 `LibHeader`가 동작하는 것을 봅시다:
...
[Angular 스타일 가이드](style-guide)에 따르면, 컴포넌트는 "Component"라는 접미사를 사용하지 않는 것을 권장합니다.
"LibHeader"와 같은 식입니다.

컴포넌트는 토큰과 관련되어 있지만 둘을 명확하게 구분해야 ㅎ바니다.
따라서, `LibHeaderToken`과 같이 `Token` 접미사를 붙이는 것을 권장합니다.
