...
<docs-callout title="트리 셰이킹과 @Service()">

`ApplicationConfig` `providers` 배열보다는 `@Service()` 데코레이터 사용을 더 권장합니다.
`@Service()`를 사용하면 최적화 툴이 트리 셰이킹을 수행할 수 있으며, 이 서비스가 실제로 애플리케이션에 사용되지 않으면 빌드 결과물에 포함하지 않습니다.
...
이제 좀 더 복잡한 경우를 살펴보기 위해 `Child` 클래스에 `FlowerService`의 프로바이더를 추가해 봅시다:
...
`Inspector`는 `Child`의 자식 컴포넌트로 구성하며, `inspector.ts` 파일에서 프로퍼티를 선언하면서 `FlowerService`와 `AnimalService`를 의존성 객체로 할당합니다:
...
`Inspector`를 `Child` `imports` 배열에 추가해야 하는 것을 잊지 마세요.
...
프로젝션된 `<app-inspector>` 에서 왜 <code>🐳</code>가 표시되는지 의문이 들 수 있습니다.
Angular의 의존성 주입 시스템은 **컴포넌트가 렌더링되는 곳** 이 아니라 **컴포넌트가 선언된 곳** 을 추적하기 때문입니다.
`<app-inspector>`는 `App`의 템플릿 안, 다르게 표현하면 `App`의 `<#VIEW> 안에 존재하기 때문에 `App`의 `viewProviders`의 영향을 받습니다.
컨텐츠가 `Child`안으로 프로젝션된다고 해도`Child`의 `viewProviders`에 등록된 <code>🐶</code>를 참조하는 것이 아니라 `App`에 등록된 <code>🐳</code>를 참조합니다.

그런데 다음 섹션을 보면, `Inspector`는 `Child`의 자식 컴포넌트이며, `Inspector`는 `<#VIEW>` 안에 존재하기 때문에, 이 때 참조하는 `AnimalService`는 강아지 <code>🐶</code> 값이 됩니다.
...
이 값은 `Child`의 `viewProviders`에 등록되어 있으며, `host`는 `inject()`를 실행할 때 사용합니다:
...
-->
<docs-pill-row>
  <docs-pill href="/guide/di/defining-dependency-providers" title="의존성 주입 프로바이더"/>
</docs-pill-row>
