...
의존성 주입(Dependency Injection, DI)은 클래스 안에서 인스턴스를 직접 생성하지 않고, 외부에서 주입 받는 방식으로 애플리케이션 전역에서 코드를 관리하고 공유하는 디자인 패턴입니다.
...
참고: `@Service` 데코레이터를 지정하면 서비스를 전역 범위에서 사용할 수 있도록 인스턴스를 싱글턴으로 생성합니다. 대부분의 경우 이런 사용 방식을 권장합니다.
...
[`inject`](/api/core/inject)를 사용할 수 있는 코드 범위를 "의존성 주입 컨텍스트(injection context)"라고 합니다.
