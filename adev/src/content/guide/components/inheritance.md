...
부모 클래스가 프로퍼티 값을 초기화 할 때 `inject()`를 사용하면, 자식 클래스도 이 프로퍼티를 자동으로 상속받습니다.
...
export class CustomListbox extends ListboxBase {
  // `ListboxBase`를 상속해서 `element` 프로퍼티를 선언합니다.
}
```

부모 클래스에서 생성자로 의존성 객체를 주입하면, 자식 클래스에서는 `super`를 사용해서 의존성 객체를 명시적으로 전달해야 합니다.
...
부모 클래스에서 `ngOnInit`과 같은 라이프싸이클 메서드를 구현했다면, 자식 클래스에서 다시 `ngOnInit` 함수를 구현하면서 부모 클래스의 메서드를 오버라이딩 할 수 있습니다.
