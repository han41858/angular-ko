...
*   `*ngIf`, `*ngFor` 안에 사용된 임베디드 뷰를 검사합니다.
*   파이프가 처리한 결과가 적절한 타입이어야 합니다.
...
`fullTemplateTypeCheck` 플래그는 Angular 13부터 지원이 중단되었습니다.
대신 `strictTemplates` 관련 컴파일 옵션을 사용하세요.
...
-->
세가지 타입 검사 모드는 각각 임베디드 뷰에서 타입을 검사하는 방식이 조금씩 다릅니다.
예제를 보면서 알아봅시다.
...
*   입력 프로퍼티의 타입이 너무 제한되었거나 적절한 메타데이터를 지정하지 않은 경우.
    `<input disabled>`와 같이 `disabled` 어트리뷰트나 불리언 값이 사용되는 어트리뷰트에서 발생할 수 있습니다.
...
*   애플리케이션 TypeScript 환경설정 파일\(`tsconfig.json`\)에서 `strictTemplates` 값을 `false`로 설정해서 엄격한 타입 검사 모드를 비활성화 하세요.
*   특정 규칙만 비활성화할 수 있습니다. 해당 *strictness flag*를 `false`로 설정하면 됩니다.
...
| `strictInputAccessModifiers` | `@Input`에 지정된 `private`/`protected`/`readonly`와 같은 접근제어자가 제대로 동작하게 합니다. `false`로 설정하면 `@Input`에 지정된 접근제어자를 무시하고 타입만 검사합니다. `strictTemplates` 값을 `true`로 지정하더라도 이 플래그의 기본값은 `false`입니다. |
...
| `strictAttributeTypes`       | 바인딩없이 문자열로 지정한 어트리뷰트도 검사할지 지정합니다. 문자열 값을 사용해서 `&lt;input matInput disabled="true"&gt;`라고 지정하거나 불리언 값을 사용해서 `&lt;input matInput [disabled]="true"&gt;`라고 지정할 수 있습니다.                    |
...
| `strictLiteralTypes`         | 템플릿에 선언된 객체와 배열 리터럴의 타입을 추론할지 지정합니다. `false`로 설정하면 이 리터럴들을 `any`로 간주합니다. `fullTemplateTypeCheck` 플래그나 `strictTemplates` 플래그를 활성화했다면 이 플래그값도 `true`로 지정됩니다.                             |
...
*   `null`이 될 수 있는 표현식에 null값 아님 보장 연산자 `!`를 사용하면 됩니다.
    ...
    그리고 `async` 파이프의 경우에는 전체 표현식을 소괄호로 감싸고 null값 아님 보장 연산자를 사용하면 됩니다.
...
*   Angular 템플릿을 검사할 때 엄격한 null 검사 모드를 해제하면 됩니다.
